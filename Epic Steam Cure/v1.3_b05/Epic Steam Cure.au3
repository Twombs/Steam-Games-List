...
; LinkReplacements(), ParseTheDRMFreeList(), ParseTheGamesList(), ParseTheHTMLGamesList(), ParseTheXMLGamesList()
; PopulateTheList(), SetStateOfControls($state)
...
Global $7zip, $a, $alternate, $appname, $array, $buttxt, $c, $chunk, $CMDfold, $code, $compress, $created, $delete, $dest
Global $display, $dlc, $download, $drmfree, $e, $end, $entries, $entry, $epic, $epicfold, $fext, $freefle, $freefle1, $freefle2
Global $freefle3, $freelist, $g, $game, $gameID, $games, $gamesfld, $gamesini, $gameslist, $htmlfld, $image, $inifle, $Legendary
Global $LEGfold, $legskip, $line, $lines, $link, $ListGUI, $logfle, $mode, $musicfld, $notes, $other, $pass, $path, $ping, $pos
Global $read, $result, $savpth, $split, $state, $SteamCMD,  $steamfold, $store, $tabs, $tera, $text, $thread, $titfle, $title
Global $URL, $user, $userID, $val, $vdffile, $version, $what, $xmlfle
...
Func MainGUI()
	...
	;
	Local $altpth, $ans, $bakfile, $cmd, $cnt, $comp, $copy, $decrypt, $decrypted, $destfld, $downfold, $drv, $encrypted
	...
	Local $pth, $savefile, $savefold, $shell, $sid, $size, $space, $style, $teracopy, $top, $type, $user32, $username, $vol
	...
	ElseIf $mode = "Epic" Then
		...
		If $epic <> 1 Then GUICtrlSetState($Button_install, $GUI_DISABLE)
		$drmfree = "Google Docs"
				...
				If $title <> "" Then
					...
					ElseIf $mode = "Epic" Then
						$ans = MsgBox(262144 + 35, "Download Query", _
							"Do you want to download & install selected game?" & @LF _
							& @LF & "YES = Download & Install." _
							& @LF & "NO = Download only (no install)." _
							& @LF & "CANCEL = Abort any download & install.", 0, $ListGUI)
							; NOTE - CANCEL could also present other options, like UPDATE a game.
						If $ans <> 2 Then
...
Func SetStateOfControls($state)
	...
	ElseIf $mode = "Epic" Then
		...
		If $epic <> 3 Then
			GUICtrlSetState($Button_id, $state)
			GUICtrlSetState($Input_id, $state)
			If $epic = 1 Then GUICtrlSetState($Button_install, $state)
		EndIf
