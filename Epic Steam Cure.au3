...
Global $display, $dlc, $done, $download, $drmfile, $drmfree, $drmlist, $drmurl, $e, $end, $entries, $entry, $epic, $epicfold, $fext
Global $freefle, $freefle1, $freefle2, $freefle3, $freelist, $g, $game, $gameID, $games, $gamesfld, $gamesini, $gameslist, $htmlfld
Global $icoI, $icoX, $image, $inifle, $keep, $Legendary, $LEGfold, $legskip, $line, $lines, $link, $ListGUI, $logfle, $manner, $mode
Global $musicfld, $nocheck, $notes, $other, $pass, $path, $ping, $pos, $process, $read, $resfile, $result, $savpth, $shell, $split
Global $state, $SteamCMD, $steamfold, $store, $tabs, $tera, $text, $thread, $timefile, $titfle, $title, $updates, $URL, $user
Global $user32, $userID, $val, $vdffile, $version, $what, $xmlfle

$CMDfold = @ScriptDir & "\SteamCMD"
$created = "September 2021."
$drmfile = @ScriptDir & "\DRMlist.ini"
$drmlist = @ScriptDir & "\DRMlist.html"
...
$version = " (v1.5 update in September 2022)"
...
Func MainGUI()
	...
	;
	Local $alias, $altpth, $ans, $bakfile, $cmd, $cnt, $comp, $copy, $decrypt, $decrypted, $destfld, $DLCtit, $downfold, $drv
	Local $encrypted, $exefile, $exStyle, $find, $gamefold, $height, $high, $icoD, $icoM, $icoS, $icoT, $idx, $ind, $left, $listurl
	Local $mass, $message, $meth, $method, $n, $notify, $num, $numb, $ontop, $params, $parent, $password, $pid, $pth, $savefile
	Local $savefold, $sid, $size, $space, $style, $t, $teracopy, $titles, $top, $type, $username, $vol, $volfile, $w, $wide, $width
	Local $winpos, $winsize, $words, $zipfile, $zipfld, $zipfold
	;
	...
	ElseIf $mode = "Epic" Then
		...
		EndIf
		;
		$nocheck = IniRead($inifle, "Epic Install", "no_check", "")
		;If $nocheck = "" Then
		;	$nocheck = 4
		;	IniWrite($inifle, "Epic Install", "no_check", $nocheck)
		;EndIf
		;
		$keep = IniRead($inifle, "Epic Install", "keep_files", "")
		;If $keep = "" Then
		;	$keep = 4
		;	IniWrite($inifle, "Epic Install", "keep_files", $keep)
		;EndIf
		;
        ...
        Select
						...
						If FileExists($destfld) Then
							...
							If $fext <> "" Then
								...
								If $mode = "Steam" Then
									...
									$image = StringLeft($image, $pos)
									$image = $image & "header.jpg"
								EndIf
								SplashTextOn("", "Saving Image!", 180, 80, -1, -1, 33)
										...
										If $ping > 0 Then
											While 1
												...
												If FileExists($savefile) Then
													...
													ExitLoop
												Else
													If $mode = "Steam" And $small <> "" Then
														SplashTextOn("", "Small Image!", 180, 80, -1, -1, 33)
														$image = $small
														$small = ""
														Sleep(1000)
													Else
							...
							If FileExists($downfold) Then
								...
								If FileExists($downfold) Then
									If $process = "install" Or $process = "download" Then
										...
										EndIf
									EndIf
								ElseIf $process = "update" Or $process = "verify" Or $process = "import" Then
								...
								If FileExists($Legendary) Then
									...
									If $ping > 0 Then
										...
										ElseIf $process = "update" Then
											; UPDATE
											$params = ' -y install ' & $appname & ' --update-only --game-folder "' & $downfold & '"'
											;$params = ' -y install ' & $appname & ' --update-only'
											;$params = ' -y install --update-only ' & $appname
										ElseIf $process = "verify" Then
											; VERIFY
											$params = ' verify-game ' & $appname
										ElseIf $process = "uninstall" Then
											; UNINSTALL
											;$params = ' -y uninstall ' & $appname
											$params = ' -y uninstall '
											If $keep = 1 Then
												$params = $params & '--keep-files '
											EndIf
										...
										ElseIf $process = "import" Then
											...
											$params = ' import-game '
											If $nocheck = 1 Then
												$params = $params & '--disable-check '
											EndIf
											$params = $params & $appname & ' "' & $downfold & '"'
										EndIf
										;
										If $process <> "verify" And $process <> "installed" And $process <> "import" Then
											If $alldlc = 1 Then
										...
										EndIf
										IniWrite($inifle, "Epic Command-Line", "parameters", $params)
										FileChangeDir($LEGfold)
										RunWait(@ComSpec & " /c legendary.exe" & $params)
										If $process = "installed" Then
										...
										Else
											If $done = "" Then
												...
												If $process = "Update" Then
													MsgBox(262192, "Process Alert", "It appears the 'Update' process did not exit cleanly." _
														& @LF & "This is usually the case when the game being checked" _
														& @LF & "is not installed, and is a probable Legendary bug.", 0, $ListGUI)
												EndIf
												$process = ""
											EndIf
										EndIf
			...
			Case $msg = $Button_notes And $buttxt = "More"
				...
				While 1
					If $notes <> "" Then
							...
							If FileExists($destfld) Then
								...
								While 1
									...
									If FileExists($savefold) Then
										...
										If FileExists($savefile) Then
											If $notes <> "" Then
												$ans = MsgBox(262144 + 33, "File Exists", "Do you wish to overwrite or add?" _
													& @LF & @LF & "OK = Overwrite." & @LF & "CANCEL = Add.", 0, $ListGUI)
												If $ans = 1 Then
						...
						ExitLoop
					Else
						$ans = MsgBox(262193 + 256, "Notes Error", "Notes don't exist for the selected game, with" & @LF & "the currently selected 'DRM-Free List' option!" _
							& @LF & @LF & "OK = Create a blank 'Notes.txt' file anyway.", 0, $ListGUI)
						If $ans = 1 Then
							$notes = 1
						Else
							ExitLoop
			...
			Case $msg = $Button_info
				...
				If $mode = "Steam" Then
					...
					$numb = "two"
				ElseIf $mode = "Epic" Then
					$message = "Of particular use, is using Legendary to download and install any" & @LF & _
						"selected game on the list, then maybe backing the game folder up" & @LF & _
						"to a zip file or self-extracting executable file, to a specified location."
				Else
				...
				EndIf
				$ans = MsgBox(262144 + 33 + 256, "Program Information", _
					...
					$notify & _
					"Third Party programs whith no ties to myself are required for some" & @LF & _
					"processes to work (i.e. 7-Zip, SteamCMD, Legendary, TeraCopy)." & @LF & @LF & _
					...
					"BIG THANKS to the developers of SteamCMD for Steam games." & @LF & _
					"BIG THANKS to the developer(s) of Legendary for Epic games." & @LF & _
					...
					"Click OK to see more information.", 0, $ListGUI)
				If $ans = 1 Then
					MsgBox(262208, "Program Information (cont.)", _
						"The FIND button has three uses, determined by the checkbox" & @LF & _
						"setting at its left. If not selected it just looks for the specified" & @LF & _
						"text in any game title on the list, selecting the first one found." & @LF & _
						"With the first of two selected settings (tick or box), each click" & @LF & _
						"will cycle through the Games list, stopping at each game that" & @LF & _
						"is listed as DRM-Free. With the second selected setting it will" & @LF & _
						"instead search the currently selected DRM-Free listing, then" & @LF & _
						"show any result in a popup dialog, which can be saved. This" & @LF & _
						"third and last use, is very handy for any name title issues, or" & @LF & _
						"for games you don't currently own at " & $mode & "." & @LF & @LF & _
						"© Created by Timboli (aka TheSaint) in " & $created & @LF & $version, 0, $ListGUI)
				EndIf
				SetStateOfControls($GUI_ENABLE)
			Case $msg = $Button_inf
								...
								For $t = 1 To $titles[0]
									...
									If StringInStr($text, $val) > 0 Then
										$ans = MsgBox(262195 + 256, "Title Results", "Found = " & $text & @LF & @LF & "Notes = " & $notes & @LF _
											...
											& @LF & "CANCEL = Abort Search." & @LF _
											& @LF & "ADVICE - To copy more than the" _
											& @LF & "title, hold down CTRL as well.", 0, $ListGUI)
										If $ans = 6 Then
											If _IsPressed("11") Then
												ClipPut($text & @CRLF & @CRLF & "Notes = " & $notes)
											Else
												ClipPut($text)
											EndIf
										ElseIf $ans = 2 Then
			...
			Case $msg = $Button_down
				...
				If $listurl <> "" Then
					If StringLeft($listurl, 4) = "http" Then
						...
						If $ping > 0 Then
							If $mode = "Epic" Then
								$drmurl = $listurl & "/gviz/tq?tqx=out:html&gid=235347284"
					...
					Else
						MsgBox(262192, "Link Error", "URL is in a wrong format!", 0, $ListGUI)
					EndIf
				EndIf
				...
				If $entry <> "" Then
					...
					If $mode = "Epic" Then
						If StringInStr($title, " - DLC - ") > 0 Then
							...
							If $DLCtit = "" And $epic = 1 Then
								$DLCtit = 1
								GUICtrlSetState($Button_options, $GUI_DISABLE)
								MsgBox(262192, "Feature Disabled", "Options for DLC entries are not yet available!", 0, $ListGUI)
							EndIf
						Else
							If $DLCtit = 1 And $epic = 1 Then
								$DLCtit = ""
								GUICtrlSetState($Button_options, $GUI_ENABLE)
							EndIf
						EndIf
					EndIf
					GUICtrlSetData($Label_free, "DRM-Free")
					;$val = IniRead($freefle, $title, "drm-free", "")
					$alias = $title
					$val = IniRead($freefle, $alias, "drm-free", "")
					If $val = "" Then
						$alias = StringReplace($alias, " - ", " ")
						$val = IniRead($freefle, $alias, "drm-free", "")
					EndIf
					If $val = "" Then
						$alias = StringReplace($alias, ": ", " ")
					...
					If $val = 1 Then
						...
						$notes = IniRead($freefle, $alias, "notes", "")
					Else
						If $mode = "Epic" Then
							$alias = $title
							$val = IniRead($drmfile, $alias, "drm", "")
							If $val = "" Then
								$alias = StringReplace($alias, " - ", " ")
								$val = IniRead($drmfile, $alias, "drm", "")
							EndIf
						...
						EndIf
						If $val = 1 Then
							GUICtrlSetData($Label_free, "DRM")
							GUICtrlSetBkColor($Label_free, $COLOR_FUCHSIA)
							GUICtrlSetColor($Label_free, $COLOR_WHITE)
							$notes = IniRead($drmfile, $alias, "notes", "")
						Else
							GUICtrlSetBkColor($Label_free, 0xFFFFCE)
							GUICtrlSetColor($Label_free, $COLOR_BLACK)
							$notes = ""
...
Func InstallOptionsEtcGUI()
	Local $Button_close, $Button_download, $Button_inform, $Button_install, $Button_installed, $Button_repair, $Button_uninstall
	Local $Button_update, $Button_verify, $Checkbox_dlc, $Checkbox_keep, $Checkbox_nope, $Checkbox_updates, $Group_install
	Local $Button_import
	;
	...
	$Button_repair = GUICtrlCreateButton("REPAIR" & @LF & "INSTALL", 130, 85, 100, 50, $BS_MULTILINE)
	...
	$Button_import = GUICtrlCreateButton("IMPORT GAME", 20, 145, 110, 32)
	GUICtrlSetFont($Button_import, 7, 600, 0, "Small Fonts")
	GUICtrlSetTip($Button_import, "Import the selected installed game!")
	$Checkbox_nope = GUICtrlCreateCheckbox("No Checking", 35, 177, 80, 20)
	GUICtrlSetTip($Checkbox_nope, "Don't check files on import!")
	$Button_uninstall = GUICtrlCreateButton("UNINSTALL", 140, 145, 90, 32)
	GUICtrlSetFont($Button_uninstall, 7, 600, 0, "Small Fonts")
	GUICtrlSetTip($Button_uninstall, "Uninstall the selected installed game!")
	$Checkbox_keep = GUICtrlCreateCheckbox("Keep Files", 153, 177, 70, 20)
	GUICtrlSetTip($Checkbox_keep, "Keep game files when uninstalling!")
	;
	...
	;
	GUICtrlSetState($Checkbox_nope, $nocheck)
        ...
        Select
			...
			Case $msg = $Button_inform
				...
				MsgBox(262208, "Install Options Information", _
					"The DOWNLOAD ONLY button does not create an 'install'" & @LF & _
					"entry in the Epic manifest, so cannot be used with other" & @LF & _
					"'Install Related' processes." & @LF & @LF & _
					"If importing an older game, you may not want file checking" & @LF & _
					"and possible updating to occur, so you can set that option." & @LF & @LF & _
					"You can uninstall a game from the manifest listing, without" & @LF & _
					"that game folder and its file content being deleted." & @LF & @LF & _
					"Most buttons will immediately close the 'Install Options'" & @LF & _
					"window and attempt that chosen process.", 0, $InstallOptionsGUI)
			Case $msg = $Button_import
				; Import the selected installed game
				$process = "import"
				$close = 1
			...
			Case $msg = $Checkbox_updates
				...
				Else
					$updates = 4
				EndIf
			Case $msg = $Checkbox_nope
				; Don't check files on import
				If GUICtrlRead($Checkbox_nope) = $GUI_CHECKED Then
					$nocheck = 1
				Else
					$nocheck = 4
				EndIf
			...
			Case $msg = $Checkbox_keep
				; Keep game files when uninstalling
...
Func ParseTheDRMFreeList()
	...
	If $read <> "" Then
		...
		ElseIf $drmfree = "Google Docs" Then
			...
			; Results & DRM List Query
			$ans = MsgBox(262145 + 48, "DRM-Free Games Result & DRM List Query", $entries & " entries found." & @LF _
				& @LF & "------------------------------" & @LF _
				& @LF & "Download the DRM List as well?" & @LF _
				& @LF & "OK = Download the DRM List." _
				& @LF & "CANCEL = Abort downloading.", 0, $ListGUI)
			If $ans = 1 Then
				$games = ""
				$download = InetGet($drmurl, $drmlist, 0, 0)
				InetClose($download)
				_FileWriteLog($logfle, "Downloaded the " & $drmfree & " DRM list.")
				If FileExists($drmlist) Then
					$lines = _FileCountLines($drmlist)
					If $lines > 0 Then
						; Process the DRM List
						$read = FileRead($drmlist)
						If $read <> "" Then
							$entries = StringSplit($read, '</tr>', 1)
							If $entries[0] > 2 Then
								_FileCreate($drmfile)
								For $e = 3 To $entries[0]
									...
									If $entry[0] > 1 Then
										...
										If $game <> "" Then
											...
											$game = StringReplace($game, "&quot;", "'")
											$game = StringReplace($game, "&amp;", "&")
											$game = StringReplace($game, "Innocense", "Innocence")
											If $games = "" Then
												$games = "[" & $game & "]" & @CRLF & "drm=1"
											Else
												$games = $games & @CRLF & "[" & $game & "]" & @CRLF & "drm=1"
											EndIf
											If $notes <> "" Then
												$games = $games & @CRLF & "notes=" & $notes
											EndIf
								...
								Next
							EndIf
							FileWrite($drmfile, $games)
							$entries = $entries[0] - 2
							MsgBox(262144, "DRM Games", $entries & " entries found.", 0, $ListGUI)
						EndIf
					Else
						_FileWriteLog($logfle, "Content Error - File is empty.")
						MsgBox(262192, "Content Error", "File is empty!", 0, $ListGUI)
